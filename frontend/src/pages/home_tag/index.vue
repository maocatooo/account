  ...
  <view>
    <uni-swipe-action>
      <view class="p-4 border-b-2 border-gray-100 shadow-md  rounded-md">
          <swipe v-for="(item, index) in tags" :key="index" :data="item" @btn1="btn1(item)" @btn2="btn2(item)">
...
import { CreateTag, Tags, UpdateTag, DeleteTag } from "../../api/index";
import { showT, toImgBase64SVG, toImgSVGByBase64, defaultIcons } from "../../api/common";
import type * as types from "../../api/types";
import { reactive, ref } from "vue";

const tags:types.Tag[] = reactive([]);
...
const btn1 = (e: types.Tag) => {
  DeleteTag(e).then(()=>{
    tags.splice(tags.findIndex(item => item.id === e.id),1)
  })
};
