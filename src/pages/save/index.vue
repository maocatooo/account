        ...
        <view class="h-80 mx-3 mt-3">
          ...
          <view class="flex flex-row items-center p-2">
            ...
            <view class="w-3/4">
              ...
              @focus="showNamePrompt = false"
                placeholder="消费金额只能是数字哦" />
            </view>
          </view>
          <view class="flex flex-row items-center p-2">
          ...
          <view class="flex overflow-x-auto text-sm p-2" v-show="showNamePrompt && (namePrompts?.length)>0">
            <view class="flex-shrink-0 px-2 " v-for="(item, index) in namePrompts" :key = index >
              <view class="whitespace-nowrap">
                <view class="px-2 py-1  rounded text-emerald-50 bg-gradient-to-r from-emerald-400 to-green-400" @click="namePromptClick(item)">
                  {{item}}
                </view>
              </view>
            </view>
          ...
          <view class="flex flex-row items-center p-2">
            ...
            <view class="w-3/4">
              <uni-easyinput   @focus="showNamePrompt = false"  :cursorSpacing="cursorSpacing" v-model="saveData.record" placeholder="备注" />
            </view>
          </view>
          <view class="flex flex-row items-center p-2">
...
import { UpdateBookJournal, CreateBookJournal, Tags, Books, NamePrompt } from "../../api/index";
...
const createBookJournal = async () => {
  ...
  if (saveData.id !== "") {
    await UpdateBookJournal(saveData)
    showT("修改成功")
  } else {
    await CreateBookJournal(saveData)
    showT("添加成功")
  }
...
onShow(async () => {
  ...
  saveData.bookID = (await Books())[0].id
  const updateData: types.BookJournal | undefined = uni.getStorageSync("updateBookJournal")
  if (updateData) {
    uni.removeStorageSync("updateBookJournal")
    saveData.bookID = updateData.bookID
